...
export type Reliability={attended_count:number;cancelled_count:number;no_show_count:number}
...
export const api={
 venues:(q='')=>request<Venue[]>(`/venues?q=${encodeURIComponent(q)}`), facility:(id:number)=>request<Facility>(`/facilities/${id}`), slots:(id:number,date:string)=>request<{date:string;slots:Slot[]}>(`/venues/${id}/slots?date=${date}`), matches:()=>request<Match[]>('/matches'), me:()=>request<User>('/me'), notifications:()=>request<Notification[]>('/notifications'), readNotification:(id:number)=>request(`/notifications/${id}/read`,{method:'POST'}), readAllNotifications:()=>request('/notifications/read-all',{method:'POST'}),
