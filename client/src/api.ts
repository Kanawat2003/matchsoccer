...
export type Venue={id:number;name:string;area:string;address:string;rating:number;price_per_hour:number;roof:number;field_types:string;owner_id?:number|null;facility_id?:number|null;facility_name?:string|null;facility_address?:string|null;facility_phone?:string|null;facility_description?:string|null;facility_image?:string|null;facility_field_count?:number;image?:string|null;service_status?:'active'|'maintenance'|'inactive';review_status?:string}
export type Facility={id:number;owner_id:number;name:string;address:string;phone:string;description:string;image:string|null;fields:Venue[]}
export type Slot={start:string;end:string;available:boolean}
export type Match={id:number;creator_id:number;title:string;venue_name:string;match_date:string;start_time:string;end_time:string;fee:number;max_players:number;players:number;booking_id:number;open_for_join:number;age_13_15?:number;age_16_18?:number;age_19_25?:number;age_26_plus?:number}
export type MatchPlayer={id:number;name:string;phone:string|null;birth_date:string|null;age:number|null;attendance_status?:'pending'|'attended'|'cancelled'|'no_show';owner:number}
export type Reliability={attended_count:number;cancelled_count:number;no_show_count:number;reliability_score:number;trust_level?:string}
export type Notification={id:number;type:string;title:string;message:string;read:number;created_at:string}
export type User={id:number;name:string;email:string;role:string;points:number;wins:number;losses:number;phone:string|null;address:string|null;avatar:string|null;birth_date:string|null;age:number|null}
...
export type AuditLog={id:number;actor_user_id:number|null;action:string;entity_type:string;entity_id:number|null;metadata:string;created_at:string;actor_name?:string|null;actor_email?:string|null}
...
export const api={
 venues:(q='')=>request<Venue[]>(`/venues?q=${encodeURIComponent(q)}`), facility:(id:number)=>request<Facility>(`/facilities/${id}`), slots:(id:number,date:string)=>request<{date:string;slots:Slot[]}>(`/venues/${id}/slots?date=${date}`), matches:()=>request<Match[]>('/matches'), me:()=>request<User>('/me'), reliabilityMe:()=>request<Reliability>('/me/reliability'), notifications:()=>request<Notification[]>('/notifications'), readNotification:(id:number)=>request(`/notifications/${id}/read`,{method:'POST'}), readAllNotifications:()=>request('/notifications/read-all',{method:'POST'}),
 login:(email:string,password:string)=>request<{token:string;user:User}>('/auth/login',{method:'POST',body:JSON.stringify({email,password})}), forgotPassword:(email:string)=>request<{message:string;devCode?:string}>('/auth/forgot-password',{method:'POST',body:JSON.stringify({email})}), resetPassword:(email:string,code:string,newPassword:string)=>request<{ok:boolean;message:string}>('/auth/reset-password',{method:'POST',body:JSON.stringify({email,code,newPassword})}), register:(name:string,email:string,password:string,phone:string,address:string,birthDate?:string)=>request<{token:string;user:User}>('/auth/register',{method:'POST',body:JSON.stringify({name,email,password,phone,address,...(birthDate?{birthDate}:{})})}), updateProfile:(data:{name:string;phone:string;address:string;avatar:string|null;birthDate?:string})=>request<User>('/me',{method:'PATCH',body:JSON.stringify(data)}),
 book:(venueId:number,bookingDate:string,startTime:string,endTime:string,totalPrice:number)=>request('/bookings',{method:'POST',body:JSON.stringify({venueId,bookingDate,startTime,endTime,totalPrice})}), bookings:()=>request<BookingRow[]>('/bookings/me'), cancelBooking:(id:number)=>request(`/bookings/${id}/cancel`,{method:'POST'}), matchesMe:()=>request<Match[]>('/matches/me'), join:(id:number)=>request(`/matches/${id}/join`,{method:'POST'}), players:(id:number)=>request<{match:{creator_id:number;max_players:number;open_for_join:number};players:MatchPlayer[]}>(`/matches/${id}/players`), leaveMatch:(id:number)=>request(`/matches/${id}/leave`,{method:'POST'}), removePlayer:(matchId:number,userId:number)=>request(`/matches/${matchId}/players/${userId}`,{method:'DELETE'}), reliability:(id:number)=>request(`/matches/${id}/reliability`), checkIn:(matchId:number,userId:number)=>request(`/matches/${matchId}/check-in/${userId}`,{method:'POST'}), finalizeAttendance:(id:number)=>request(`/matches/${id}/finalize-attendance`,{method:'POST'}),
 createMatch:(bookingId:number,title:string,fee:number,maxPlayers:number)=>request('/matches',{method:'POST',body:JSON.stringify({bookingId,title,fee,maxPlayers})}), openMatch:(id:number)=>request(`/matches/${id}/open`,{method:'POST'}), closeMatch:(id:number)=>request(`/matches/${id}/close`,{method:'POST'}), ownerVenues:()=>request<Venue[]>('/owner/venues'), ownerBookings:()=>request<OwnerBookingRow[]>('/owner/bookings'), ownerFacilities:()=>request<Facility[]>('/owner/facilities'), createFacility:(data:Partial<Facility>)=>request<Facility>('/owner/facilities',{method:'POST',body:JSON.stringify(data)}), updateFacility:(id:number,data:Partial<Facility>)=>request<Facility>(`/owner/facilities/${id}`,{method:'PATCH',body:JSON.stringify(data)}), createField:(facilityId:number,data:Record<string,unknown>)=>request<Venue>(`/owner/facilities/${facilityId}/fields`,{method:'POST',body:JSON.stringify(data)}), adminUsers:()=>request<User[]>('/admin/users'), adminAuditLogs:(limit=30)=>request<AuditLog[]>(`/admin/audit-logs?limit=${limit}`), updateAdminRole:(id:number,role:string)=>request<User>(`/admin/users/${id}/role`,{method:'PATCH',body:JSON.stringify({role})}), assignVenueOwner:(id:number,ownerId:number|null)=>request<Venue>(`/admin/venues/${id}/owner`,{method:'PATCH',body:JSON.stringify({ownerId})}), updateOwnerVenue:(id:number,data:Partial<Pick<Venue,'name'|'area'|'address'|'price_per_hour'|'roof'|'field_types'|'image'|'service_status'>>)=>request<Venue>(`/owner/venues/${id}`,{method:'PATCH',body:JSON.stringify(data)}),
