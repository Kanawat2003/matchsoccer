...
const host={name:`E2E Host ${stamp}`,email:`e2e-host-${stamp}@porsball.test`,password:'TestPass123!',phone:'0812345678',address:'E2E Test Address'}
const guest={name:`E2E Guest ${stamp}`,email:`e2e-guest-${stamp}@porsball.test`,password:'TestPass123!',phone:'0812345678',address:'E2E Test Address'}
let H,G
...
async function main(){
 ...
 const second=await findAvailableSlot(2,2)
 const b2=await call('booking for cancel','/bookings',{method:'POST',token:H.token,body:JSON.stringify({venueId:2,bookingDate:second.date,startTime:second.slot.start,endTime:second.slot.end,totalPrice:900})})
